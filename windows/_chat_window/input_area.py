...
class SendButton(QPushButton):
    def __init__(self):
        ...
        qss_formatter.add_widget(self, 'InputSendButton', QSSFiles.chat_window)
...
class InputTextEdit(QTextEdit):
    def __init__(self):
        ...
        qss_formatter.add_widget(self, 'InputTextedit', QSSFiles.chat_window)
        self.setSizePolicy(
    ...
    def input_textedit_ondocuentSizeChanged(self, size):  # 用来约束文本框使得高度随内容自适应
        if size.height() < qss_formatter.config.chat_window.max_input_area_height:
            ...
            self.setFixedHeight(qss_formatter.config.chat_window.max_input_area_height)
...
class InputArea(QWidget):
    ...
    def __init__(self):
        super().__init__()
        # ----------- 文本输入区域 ------------------------------------------------------------------
        qss_formatter.add_widget(self, 'InputArea', QSSFiles.chat_window)
        self.setSizePolicy(
            self.sizePolicy().horizontalPolicy(),
            QSizePolicy.Policy.Minimum
        )
        ...
        # ----------- 文本输入框 ------------------------------------------------------------------
        self.textedit = InputTextEdit()  # 文本输入部分
