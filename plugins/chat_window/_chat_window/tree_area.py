    ...
    def draw(self, root_node: Node, leaves: list[Node]):
        ...
        def update_table(node: Node, deepth: int):
            if node not in table:
                table[node] = TreeNode(node.message.role, node.uuid, deepth, 
                                         self.formatter.chat_window_tree_node_radiu,
                                         self.formatter.chat_window_tree_node_space,
                                         self.formatter
                                         )
        ...
        def line_to_children(node: Node):
            for child in node.children:
                ...
                line.setPen(QPen(
                    QColor(self.formatter.chat_window_tree_line_color),
                    self.formatter.chat_window_tree_node_line_width)
                )
                line.setZValue(0)
