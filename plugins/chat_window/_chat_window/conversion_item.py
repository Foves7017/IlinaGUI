...
class ConversionItem(QWidget):
    ...
    def __init__(self, node_uuid: UUID, message: IlinaMessage, formatter: Formatter):
        super().__init__()
        ...
        # 从设置中读取自动折叠高度备用
        self.max_collapse_height = ConfigLoader(app_dir()/'configs'/'window.json', ChatWindowConfig).readonly().max_collapse_height

        formatter.add_qss_widget(self, QSS_FILEPATH, 'ConversionItem')
        ...
        self.top_expand_button.setObjectName('ConversionItemButton')
        ...
        self.top_expand_reasoning_button.setObjectName('ConversionItemButton')
        self.top_expand_reasoning_button.pressed.connect(self.on_reason_expand_pressed)

        # 顶部按钮区
        self.top_button_area = QWidget()
        self.top_button_area.setObjectName('ConversionItemButtonArea')
