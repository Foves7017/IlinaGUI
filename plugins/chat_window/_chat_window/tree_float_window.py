...
from ..consts import ChatWindowConfig, QSS_FILEPATH
...
class TreeFloatWindow(QWidget):
    ...
    def __init__(self, formatter: Formatter):
        ...
        self.role_label.setObjectName('TreeFloatWindowRole')

        # 显示内容
        self.content_label = QLabel()
        self.content_label.setWordWrap(True)
        self.content_label.setObjectName('TreeFloatWindowContent')

        # 设置宽度
        self.content_label.setFixedWidth(formatter.chat_window_float_window_fixed_width)
    ...
    def paintEvent(self, event):
        ...
        radius = self.formatter.chat_window_float_window_radiu

        # ── 确定背景颜色与透明度 ────────────────────────────────────
        # 基础颜色取自 ROLE_COLOR_MAP，根据当前消息角色（user/assistant/tool/system）
        bg_color = QColor(self.formatter.chat_window_float_window_background_color)
        # bg_color = QColor(ROLE_COLOR_MAP[self.role])
        if not self.Clicked:
            # 未点击时：半透明，透明度从配置读取（如 128，即 50%）
            # 这样鼠标 hover 经过的浮窗不会完全遮挡下方内容
            bg_color.setAlpha(self.formatter.chat_window_float_window_background_alpha)
        else:
        ...
        for i in range(shadow_margin):
            ...
            # 这种线性衰减让阴影从内到外自然淡出
            alpha = int(
                self.formatter.chat_window_float_window_shadow_start_alpha * (1 - i / shadow_margin)
            )

            # 阴影颜色 = 角色色 + 当前层透明度
            shadow_color = QColor(getattr(self.formatter, f'role_{self.role}_extra_color'))
