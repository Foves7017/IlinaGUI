...
class TreeNode(QGraphicsObject):
    ...
    def __init__(self, node_role: IlinaMessageRoles, node_uuid, layer: int, radiu: int, space: int, formatter: Formatter):
        super().__init__()
    ...
    def paint(self, painter: QPainter, option: QStyleOptionGraphicsItem, widget=None) -> None:
        ...
        if self.columnpos == 0:
            ...
            color = QColor(getattr(self.formatter, f'role_{self.node_role}_extra_color'))
        else:
            # color = QColor(qss_formatter.get_role_color(self.node_role, True))
            color = QColor(getattr(self.formatter, f'role_{self.node_role}_normal_color'))
        ...
        if option.state & QStyle.StateFlag.State_MouseOver:
            ...
            painter.setPen(QPen(QColor(self.formatter.chat_window_tree_line_color), 1))
