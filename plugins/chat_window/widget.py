...
from PySide6.QtWidgets import QSplitter, QSizePolicy, QWidget, QVBoxLayout, QMessageBox, QLabel
...
class ContentWidget(QWidget):
    def __init__(self, init_param: InitParam):
        super().__init__()
        self.formatter = init_param.formatter
        self.root_layout = QVBoxLayout(self)

        self.log = getLogger(f'对话窗口 {str(init_param.uuid).split('-')[-1]}')

        # 加载中 Label
        self.loading_label = QLabel(text='Now Loading...')
        ...
        if init_param.open_file is None:
            self.loading_label.setText(f'需要传入一个文件')
            return
        else:
            self.filename = init_param.open_file
    ...
    def _setup_content(self):
        ...
        self.root_layout.insertWidget(0, self.state_label)

        # 创建浮动窗口
        self.tree_float_window = TreeFloatWindow(self.formatter)
    ...
    def _setup_engine(self):
        ...
        self.formatter.add_qss_widget(self, QSS_FILEPATH)
    ...
    def _setup_splitter(self):
        ...
        self.formatter.add_qss_widget(self.splitter, QSS_FILEPATH, 'Splitter')

        # 创建左侧的树视图
        self.tree_area = TreeArea(self.formatter)
        ...
        config = ConfigLoader(CONFIG_PATH, ChatWindowConfig).readonly()
    ...
    def _setup_talk_area(self):
        ...
        self.formatter.add_qss_widget(self.talk_area, QSS_FILEPATH)
        ...
        # 创建滚动区
        self.scroll_area = ScrollArea(self.formatter)
        talk_area_layout.addWidget(self.scroll_area)

        # 创建输入区
        self.input_area = InputArea(self.formatter)
    ...
    def on_splitter_moved(self):  # 记录Splitter的位置
        with ConfigLoader(CONFIG_PATH, ChatWindowConfig) as conf:
